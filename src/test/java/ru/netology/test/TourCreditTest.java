...
public class TourCreditTest {
    ...
    void allFieldsValid() {
        ...
        var creditPage = tourPage.clickCreditButton();
        creditPage.orderTourCredit(ownerCardInfo);
        creditPage.findNotification("Успешно");
        assertEquals(DataHelper.getFirstCardInfo().getCardStatus(), SQLHelper.getCreditStatus());
    }
    ...
    void invalidCardNumber() {
        ...
        var creditPage = tourPage.clickCreditButton();
        creditPage.orderTourCredit(ownerCardInfo);
        creditPage.findNotification("Ошибка");
        assertEquals(DataHelper.getSecondCardInfo().getCardStatus(), SQLHelper.getCreditStatus());
    }
    ...
    void invalidCardShortNumber() {
        ...
        var creditPage = tourPage.clickCreditButton();
        creditPage.orderTourCredit(ownerCardInfo);
        creditPage.findError("Неверный формат");
    }
    ...
    void invalidSmallerMonth() {
        ...
        var creditPage = tourPage.clickCreditButton();
        creditPage.orderTourCredit(ownerCardInfo);
        creditPage.findError("Неверно указан срок действия карты");
    }
    ...
    void invalidSmallerYear() {
        ...
        var creditPage = tourPage.clickCreditButton();
        creditPage.orderTourCredit(ownerCardInfo);
        creditPage.findError("Истёк срок действия карты");
    }
    ...
    void invalidOneLetterName() {
        ...
        var creditPage = tourPage.clickCreditButton();
        creditPage.orderTourCredit(ownerCardInfo);
        creditPage.findError("Поле должно содержать от 2 до 64 латинских букв");
    }
    ...
    void invalidDashBeforeName() {
        ...
        var creditPage = tourPage.clickCreditButton();
        creditPage.orderTourCredit(ownerCardInfo);
        creditPage.findError("Дефис не может стоять перед/после имени/фамилии");
    }
    ...
    void invalidSpaceName() {
        ...
        var creditPage = tourPage.clickCreditButton();
        creditPage.orderTourCredit(ownerCardInfo);
        creditPage.findError("Поле обязательно для заполнения");
    }
